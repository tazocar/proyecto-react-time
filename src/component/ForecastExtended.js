...
class ForecastExtended extends Component {
  ...
  renderForcastItemDay(forecastData) {
    return forecastData.map(forecast =>(
      <ForecastItem
        key={`${forecast.weekDay}${forecast.hour}`} weekDay={forecast.weekDay} hour={forecast.hour} data={forecast.data}>
      </ForecastItem>
      ))
    //return 'Render Items';
    /*
  ...
  render() {
    ...
    return (
      <div>
        ...
        {forecastData !== null ? this.renderForcastItemDay(forecastData) : this.renderProgress()}
      </div>
    )
  }
}
